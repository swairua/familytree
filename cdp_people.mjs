...
// Capture the Individuals list page JSON to get all people + data.
// People page may paginate; we'll scroll to load more and capture all JSON.
...
await send('Network.enable', { maxTotalBufferSize: 100 * 1024 * 1024, maxResourceBufferSize: 100 * 1024 * 1024 });
...
on('Network.responseReceived', (params) => {
  try {
    ...
    if (response.status >= 200 && response.status < 400 && (ct.includes('json') || /api|graphql|people|individual|person/i.test(url))) {
...
on('Network.loadingFinished', async (params) => {
  try {
    ...
    const short = meta.url.split('?')[0].substring(0, 110);
    console.log(`[CAP] ${meta.status} ${meta.ct} ${short} (${body.length})`);
  } catch {}
});

// Navigate to Individuals list (all trees)
console.log('\nNavigating to Individuals page...');
await send('Page.navigate', {
  url: 'https://www.myheritage.com/people-OYYV6UYCQOJ76UUGSORCEA4K7X53VLQ/wairua?query=&scope=all&treeId=all',
});
...
const state = await send('Runtime.evaluate', {
  expression: `JSON.stringify({
    ...
    personRows: Array.from(document.querySelectorAll('[data-person-id], .person-row, [class*="person"], [class*="individual"]')).length,
...
try {
  ...
  console.log('person-like rows:', val.personRows);
  console.log('inputs:', JSON.stringify(val.inputs));
  console.log('TEXT:', val.text.replace(/\s+/g, ' ').substring(0, 1000));
...
netCaptures.forEach((c, i) => {
  try {
    ...
    fs.writeFileSync(`${OUT_DIR}/people_${String(i).padStart(3, '0')}.json`, JSON.stringify({ url: c.url, ct: c.ct, status: c.status, data: parsed }, null, 2));
  } catch {
    fs.writeFileSync(`${OUT_DIR}/people_${String(i).padStart(3, '0')}.json`, JSON.stringify({ url: c.url, ct: c.ct, status: c.status, body: c.body.substring(0, 3000) }, null, 2));
  }
});
console.log('Saved people captures.');
