    ...
    for (const child of line.children) {
      switch (child.tag) {
        ...
        case 'OBJE':
          ...
          // Try to extract photo from inline OBJE record
          for (const sub of child.children) {
            if (sub.tag === 'FILE') {
              individual.photo = sub.value
            } else if (sub.tag === 'BLOB') {
              individual.photo = sub.value
            }
          }
          break
        case '_PHOTO':
          // MyHeritage-specific photo tag
          for (const sub of child.children) {
            if (sub.tag === 'FILE') {
              individual.photo = sub.value
            } else if (sub.tag === 'BLOB') {
          ...
          if (!individual.photo && child.value) {
            individual.photo = child.value
          }
          break
        case 'FAMC':
